...
# Tab 0: 诊断
with tabs[0]:
    c_in = st.text_input("分析并入库", "600519")
    if st.button("开始单股分析"):
...
with tabs[1]:
    ...
    if st.button("开始增量基建普查", type="primary"):
        with st.spinner("正在执行基建..."):
            pool = [f"{p}{i:03d}" for p in ["600","601","603","000","002"] for i in range(500)]
            existing = set(db["code"].astype(str))
            todo = [c for c in pool if c not in existing][:400]
...
with tabs[7]:
    st.header("🚩 趋势线蓄势")
    st.info("墨菲下降通道引擎待完全迁移...")

st.sidebar.success("✅ 应用已启动")
