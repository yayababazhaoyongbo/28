...
st.caption("✅ 数据源已切换为 Akshare，可正常抓取")

db_manager = DatabaseManager()
db = db_manager.load_db()

st.sidebar.metric("基因库总量", f"{len(db)} 只")
st.sidebar.info(f"并发线程: {MAX_WORKERS} | Akshare 数据源")

# ...（后续 Tabs 代码与上一版相同，可继续使用我之前提供的完整 Tabs 部分）
...
with tabs[0]:
    ...
    if st.button("单股诊断", type="primary"):
        with st.spinner("正在通过 Akshare 获取数据..."):
            result = SoulEngine.get_data(c_in)
            if result[0] is not None:
                ...
                if success:
                    st.success(f"✅ 【{name_d} ({c_in})】入库成功！最佳灵魂均线：{ma}日")
                    st.rerun()
            else:
                st.error(f"❌ 抓取失败：{result[1]}")

# （基建系统和其他策略 Tab 可继续使用上一版代码）

st.sidebar.success("✅ Akshare 已就绪，可开始基建扫描")
